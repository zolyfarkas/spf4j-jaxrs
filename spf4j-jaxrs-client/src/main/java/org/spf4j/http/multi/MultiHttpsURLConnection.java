...
public final class MultiHttpsURLConnection extends HttpsURLConnection {
  ...
  public MultiHttpsURLConnection(final URL url, final int choiceCount) throws MalformedURLException  {
    super(url);
    this.urls = parseURLs(url);
    choice = choiceCount % urls.length;
    try {
      this.current = (HttpsURLConnection) this.urls[choice].openConnection();
  ...
  private void connect(final int attempt, final IOException ex) throws IOException {
    ...
    try {
      ...
      current.setSSLSocketFactory(super.getSSLSocketFactory());
      current.setHostnameVerifier(super.getHostnameVerifier());
      current.connect();
    } catch (IOException e) {
       nextUrl();
       connect(attempt + 1, e);
  ...
  public synchronized void nextUrl() throws IOException {
    this.current = (HttpsURLConnection) urls[++choice % urls.length].openConnection();
  }
