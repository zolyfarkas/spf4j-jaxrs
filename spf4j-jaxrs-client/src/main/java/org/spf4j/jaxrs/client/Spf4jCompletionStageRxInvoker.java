...
public final class Spf4jCompletionStageRxInvoker
  ...
  private <T> CompletionStage<T> submit(final Callable<T> what, final String name) {
    ...
    Callable<T> pc = Spf4jInvocation.invocationHandler(current, what, name,
            deadlineNanos, invocationBuilder.getHttpReqTimeoutNanos(), invocationBuilder.getTarget().getClient());
  ...
  public <T> CompletionStage<T> get(final GenericType<T> responseType) {
     return submit(() -> invocationBuilder.buildGet().getWrapped().invoke(responseType), getName(HttpMethod.GET));
  }

  @Override
  public CompletionStage<Response> put(final Entity<?> entity) {
    return submit(() -> invocationBuilder.buildPut(entity).getWrapped().invoke(), getName(HttpMethod.PUT));
  }

  @Override
  public <T> CompletionStage<T> put(final Entity<?> entity, final Class<T> clazz) {
    return submit(() -> invocationBuilder.buildPut(entity).getWrapped().invoke(clazz), getName(HttpMethod.PUT));
  }

  @Override
  public <T> CompletionStage<T> put(final Entity<?> entity, final GenericType<T> type) {
    return submit(() -> invocationBuilder.buildPut(entity).getWrapped().invoke(type), getName(HttpMethod.PUT));
  }

  @Override
  public CompletionStage<Response> post(final Entity<?> entity) {
    return submit(() -> invocationBuilder.buildPost(entity).getWrapped().invoke(), getName(HttpMethod.POST));
  }

  @Override
  public <T> CompletionStage<T> post(final Entity<?> entity, final Class<T> clazz) {
    return submit(() -> invocationBuilder.buildPost(entity).getWrapped().invoke(clazz), getName(HttpMethod.POST));
  }

  @Override
  public <T> CompletionStage<T> post(final Entity<?> entity, final GenericType<T> type) {
    return submit(() -> invocationBuilder.buildPost(entity).getWrapped().invoke(type), getName(HttpMethod.POST));
  }

  @Override
  public CompletionStage<Response> delete() {
    return submit(() -> invocationBuilder.buildDelete().getWrapped().invoke(), getName(HttpMethod.DELETE));
  }

  @Override
  public <T> CompletionStage<T> delete(final Class<T> responseType) {
    return submit(() -> invocationBuilder.buildDelete().getWrapped().invoke(responseType), getName(HttpMethod.DELETE));
  }
  ...
  public CompletionStage<Response> head() {
    return submit(() -> invocationBuilder.build(HttpMethod.HEAD).getWrapped().invoke(), getName(HttpMethod.HEAD));
  }

  @Override
  public CompletionStage<Response> options() {
    return submit(() -> invocationBuilder.build(HttpMethod.OPTIONS).getWrapped().invoke(), getName(HttpMethod.OPTIONS));
  }
  ...
  public <T> CompletionStage<T> trace(final Class<T> responseType) {
    return submit(() -> invocationBuilder.build("TRACE").getWrapped().invoke(responseType), getName("TRACE"));
  }
  ...
  public CompletionStage<Response> method(final String name) {
    return submit(() -> invocationBuilder.build(name).getWrapped().invoke(), getName(name));
  }

  @Override
  public <T> CompletionStage<T> method(final String name, final Class<T> responseType) {
    return submit(() -> invocationBuilder.build(name).getWrapped().invoke(responseType), getName(name));
  }
  ...
  public CompletionStage<Response> method(final String name, final Entity<?> entity) {
     return submit(() -> invocationBuilder.build(name, entity).getWrapped().invoke(), getName(name));
  }

  @Override
  public <T> CompletionStage<T> method(final String name, final Entity<?> entity, final Class<T> responseType) {
    return submit(() -> invocationBuilder.build(name, entity).getWrapped().invoke(responseType), getName(name));
  }
